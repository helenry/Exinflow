...
print('apakah didispose1');
    Get.delete<TabController>();
print('apakah didispose2');
    super.dispose();
print('apakah didispose3');
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: TopBar(
        ...
        type: '',
        from: '',
        // subtype: subtabController.selectedTab.value == 0 ? 'transaction' : subtabController.selectedTab.value == 1 ? 'template' : 'plan',
        subtype: subtabController.selectedTab.value == 0 ? 'transaction' : 'plan',
        subIndex: -1
      ),
      ...
      body: SingleChildScrollView(
        child: AllPadding(
          child: Obx(() {
            return Column(
              children: [
                Subtab(tabs: tabs, controller: transactionsTabController),
                Padding(
                ...
                StreamBuilder<QuerySnapshot>(
                  // stream: FirebaseFirestore.instance.collection(subtabController.selectedTab.value == 0 ? 'Transactions' : subtabController.selectedTab.value == 1 ? 'Transaction_Templates' : 'Transaction_Plans').where('User', isEqualTo: user?.uid ?? '').where('Is_Deleted', isEqualTo: false).snapshots(),
                  stream: FirebaseFirestore.instance.collection(subtabController.selectedTab.value == 0 ? 'Transactions' : 'Transaction_Plans').where('User', isEqualTo: user?.uid ?? '').where('Is_Deleted', isEqualTo: false).snapshots(),
                  builder: (context, snapshot) {
                    ...
                    if(subtabController.selectedTab.value == 0 && check.data()!.containsKey('Date')) {
                    ...
                    return ListView.builder(
                      ...
                      itemCount: subtabController.selectedTab.value == 0 && check.data()!.containsKey('Date') ? groupedItems.length : snapshot.data!.docs.length,
                      itemBuilder: (context, index) {
                        var doc = null;
                        doc = snapshot.data!.docs[index];

                        Future<Map<String, dynamic>> conversionRates = currencyService.conversionRate(mainCurrency, currencyController.usedCurrencies ?? [], subtabController.selectedTab.value == 0 && doc.data()!.containsKey('Date') ? DateFormat('yyyy-MM-dd').format(DateFormat('d MMMM yyyy', 'id_ID').parse(groupedItems.keys.elementAt(index))) : 'now');
                        ...
                        return FutureBuilder<Map<String, dynamic>>(
                          ...
                          builder: (context, futureSnapshot) {
                            ...
                            return Container(
                              ...
                              margin: EdgeInsets.only(bottom: (index + 1 != (subtabController.selectedTab.value == 0 && check.data()!.containsKey('Date') ? groupedItems.length : snapshot.data!.docs.length)) ? 10 : 0),
                              ...
                              child: Column(
                                children: [
                                  ...
                                  // if(subtabController.selectedTab.value == 1 && doc.data()!.containsKey('Name'))
                                  //   InkWell(
                                  //     onTap: () async {
                                  ...
                                  // if(subtabController.selectedTab.value == 2 && doc.data()!.containsKey('Is_Active'))
                                  if(subtabController.selectedTab.value == 1 && doc.data()!.containsKey('Is_Active'))
                                    InkWell(
                                      onTap: () {
